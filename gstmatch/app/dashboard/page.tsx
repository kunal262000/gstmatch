...
export default function DashboardPage() {
  ...
  const fetchResults = async () => {
    ...
    try {
      ...
      // Recalculate stats after fetching data - no more hardcoded fallbacks
  ...
  // Aggregate stats across all user reconciliations (using actual stored results)
  const stats = useMemo(() => {
    let totalInvoices = 0
    let totalMatched = 0
    let totalAtRisk = 0
    ...
    return {
      ...
      matchAccuracy: totalInvoices ? Math.round((totalMatched / totalInvoices) * 100) : 0,
    }
  }, [rows])
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: '12px', fontWeight: 700, color: '#10b981' }}>
              {stats.totalRecons > 0 && '+5 vs last month'}
            </div>
          </div>
          ...
          >
            ...
            <div style={{ fontSize: '12px', fontWeight: 700, color: '#10b981' }}>
              {stats.totalRecons > 0 && '+18% vs last month'}
            </div>
          </div>
          ...
          >
            ...
            <div style={{ fontSize: '12px', fontWeight: 700, color: '#ef4444' }}>
              {stats.totalInvoices > 0 && '18.1% of total ITC'}
            </div>
          </div>
        </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
              ...
              >
                ...
                >
                  ...
                  <span style={{ fontSize: '12px', fontWeight: 800, color: '#1e293b' }}>₹{stats.totalRecovered + stats.totalAtRisk > 0 ? (stats.totalRecovered + stats.totalAtRisk).toLocaleString('en-IN') : '0'}</span>
                </div>
              </div>
              ...
              <div style={{ fontSize: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  <span style={{ color: '#1e293b', fontWeight: 600 }}>Recovered</span>
                </div>
                {stats.totalAtRisk > 0 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#ef4444' }} />
                    <span style={{ color: '#1e293b', fontWeight: 600 }}>At Risk: ₹{stats.totalAtRisk.toLocaleString('en-IN')}</span>
                  </div>
                )}
                {stats.totalInvoices > 0 && stats.totalRecovered > 0 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#f59e0b' }} />
                    <span style={{ color: '#1e293b', fontWeight: 600 }}>Not Eligible</span>
                  </div>
                )}
              </div>
            </div>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
              ...
              >
                ...
                >
                  <span style={{ fontSize: '18px', fontWeight: 900, color: '#10b981' }}>{(stats.complianceScore || 0)}%</span>
              ...
              <div style={{ fontSize: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#10b981' }} />
                  <span style={{ color: '#1e293b', fontWeight: 600 }}>Good (80-100): {(stats.matchAccuracy || 0)}%</span>
                </div>
                {stats.matchAccuracy < 80 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#f59e0b' }} />
                    <span style={{ color: '#1e293b', fontWeight: 600 }}>Average (50-79): {Math.max(0, 80 - stats.matchAccuracy || 0)}%</span>
                  </div>
                )}
                {stats.matchAccuracy < 50 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#ef4444' }} />
                    <span style={{ color: '#1e293b', fontWeight: 600 }}>Low (0-49): {Math.max(0, 50 - stats.matchAccuracy || 0)}%</span>
                  </div>
                )}
              </div>
            </div>
