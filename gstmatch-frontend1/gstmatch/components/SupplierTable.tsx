  ...
  return (
    <div>
      ...
      <div className="neu-raised" style={{ overflow: 'hidden', padding: 0 }}>
        ...
        <div style={{
          display: 'grid',
          gridTemplateColumns: '2fr 1.5fr 80px 120px 100px',
          padding: '10px 18px',
          borderBottom: '1px solid var(--neu-dark)',
        }}>
          {['Supplier', 'GSTIN', 'Invoices', 'Status', 'ITC at risk'].map(h => (
            <span key={h} style={{
          ...
          filtered.map((s, i) => (
            ...
            >
              ...
              <div style={{
                ...
                color: s.itcAtRisk > 0 ? 'var(--danger)' : 'var(--primary)',
              }}>
                {s.itcAtRisk > 0 ? `₹${s.itcAtRisk.toLocaleString('en-IN')}` : '—'}
              </div>
            </div>
          ))
        )}
